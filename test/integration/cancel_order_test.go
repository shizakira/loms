...
func (s *Suite) Test_OrderCancel_Success() {
	...
	_, err = s.OrderCancel(context.Background(), &pb.OrderCancelRequest{
	...
	s.NoError(err)
	s.Equal("cancelled", info.GetStatus())
}

func (s *Suite) Test_OrderCancel_NotFound() {
	_, err := s.OrderCancel(context.Background(), &pb.OrderCancelRequest{
		OrderId: 99999999,
	})
...
func (s *Suite) Test_OrderCancel_InvalidStatus() {
	created, err := s.OrderCreate(context.Background(), &pb.OrderCreateRequest{
		User: 1,
		Items: []*pb.Item{
			{Sku: 1076963, Count: 1},
	...
	_, err = s.OrderCancel(context.Background(), &pb.OrderCancelRequest{OrderId: created.GetOrderId()})
